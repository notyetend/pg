...
function ret = loss(model, data, wd_coefficient)
  % model.input_to_hid is a matrix of size <number of hidden units> by <number of inputs i.e. 7>. It contains the weights from the input units to the hidden units.
  ...
  % data.inputs is a matrix of size <number of inputs i.e. 7> by <number of data cases>. Each column describes a different data case. 
...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  % model.input_to_hid is a matrix of size <number of hidden units> by <number of inputs i.e. 7>
...
function ret = theta_to_model(theta)
  ...
  n_hid = size(theta, 1) / (7+10);
  ret.input_to_hid = transpose(reshape(theta(1: 7*n_hid), 7, n_hid));
  ret.hid_to_class = reshape(theta(7 * n_hid + 1 : size(theta,1)), n_hid, 10).';
end
