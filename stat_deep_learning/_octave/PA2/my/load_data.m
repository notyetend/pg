...
if debug
  M = floor(debug_data_size / N);
  train_input = reshape(mod(data.trainData(1:D, 1:N * M), debug_vocab_size) .+ 1, D, N, M);
  train_target = reshape(mod(data.trainData(D + 1, 1:N * M), debug_vocab_size) .+ 1, 1, N, M);
  valid_input = mod(data.validData(1:D, debug_data_size), debug_vocab_size) .+ 1;
  valid_target = mod(data.validData(D + 1, debug_data_size), debug_vocab_size) .+ 1;
  test_input = mod(data.testData(1:D, debug_data_size), debug_vocab_size) .+ 1;
  test_target = mod(data.testData(D + 1, debug_data_size), debug_vocab_size) .+ 1;
  vocab = data.vocab(1:debug_vocab_size);
else
