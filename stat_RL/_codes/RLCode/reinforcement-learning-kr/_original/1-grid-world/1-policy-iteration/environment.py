...
class GraphicDisplay(tk.Tk):
    ...
    def rectangle_move(self, action):  # 6
    ...
    def draw_one_arrow(self, col, row, policy):  # 9
        if col == 2 and row == 2:
            return
    ...
    def draw_from_policy(self, policy_table):  # 10
        for i in range(HEIGHT):
            for j in range(WIDTH):
                self.draw_one_arrow(i, j, policy_table[i][j])

    def print_value_table(self, value_table):  # 11
