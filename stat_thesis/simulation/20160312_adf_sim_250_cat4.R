...
# Data file path
#ch_fileName <- "D:/9000_etc/Thesis/data/dac_sample.tar/dac_sample_small.txt"
#ch_fileName <- "D:/9000_etc/Thesis/data/dac.tar/train.txt"
ch_fileName <- "C:/Temp/dac.tar/train.txt" # file on SSD

# Loading table takes few minutes, so run below code only when you need to.
#dat <- read.table(ch_fileName, header = FALSE, sep = '\t')
#names(dat) <- li_fiendnames


# Start for checking of running time
ptm <- proc.time()
...
for( i in 1:nu_line ) {
  ...
  if(i %% 1000 == 0) {
    #print(df_oneLine)
    print(paste(i, y, p_new, log_loss_adf/i, sep=', '))
  }
...
# End for checking of running time
proc.time() - ptm
